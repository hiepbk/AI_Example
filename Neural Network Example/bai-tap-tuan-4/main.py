...
# -*- coding: utf-8 -*-
"""
Created on Thu May  6 23:17:54 2021
...
#%% define activation funtion sigmoid, relu, tanh
def sigmoid(x):
  return 1/(1 +np.exp(-x))
def tanh(x):
  return np.tanh(x)
...
def sigmoid_derivative(x):
  ...
  return ds
x = np.array([1, 2, 3])
dao_ham_sigmoid = sigmoid_derivative(x)
#%% 
...
def initialize_with_zeros(dim):
    ...
    return w, b
#%% Propagate Ham lan truyen xuoi nguoc
...
model = model(train_set_x,train_set_y,test_set_x,test_set_y,2000,0.001,print_cost=True)
